...
::使用方法：
::首先在自定义区里设置相关配置。
::然后在主bat将 choice_syxz 值设为 系统，Fastboot，Recovery 或 ADBSideload。此为选择检测项目。
...
::自定义区
::log.txt路径
set logpath_syxz=chkconnect_log.txt
::最大重试次数
set maxretries_syxz=15
::每两次重试之间的等待时间（秒）
set wait_syxz=1
...
:CHKCONNECT_PRE
if "%choice_syxz%"=="系统" set keyword_syxz=	device
...
if not "%keyword_syxz%"=="blank" ECHO.正在检测设备连接（%choice_syxz%）... & goto CHKCONNECT_START
set /p choice_syxz=未（正确）指定检测项目！请输入检测项目（系统，Fastboot，Recovery或ADBSideload）：
goto CHKCONNECT_PRE
...
if "%devnum_syxz%"=="1" set result_syxz=1& ECHO.设备已连接（%choice_syxz%）！ & goto :eof
set /a times_syxz+=1
...
:CHKCONNECT_FAILED
set result_syxz=0
if "%devnum_syxz%"=="0" ECHO.未检测到设备（%choice_syxz%）！ & goto :eof
if %devnum_syxz% GTR 1 ECHO.有多个设备连接（%choice_syxz%）！请断开其他设备！ & goto :eof
ECHO.未知错误！ & goto :eof
